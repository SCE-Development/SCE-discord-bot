...
module.exports = new Command({
  ...
  let queryThread = await FIND_THREAD(data);
    ...
    switch (queryThread.responseData.length) {
      case 0:   await createNewThread(data, message); 
                break;
      case 1:   await addMessageToThread(data, message, queryThread);
                break;
      default:  await multipleThreadResults(data, message, queryThread);
    }
}
});
/** 
 * Below is all the supporting functions
 * @param {enumeration} data an enumeration containing all necessary data to do APIfunction calls, and make embeds
 * @param {Discord.Message} message the original message that started this command
 **/
async function createNewThread(data, message){
  ...
  if(confirmed)
  {
      const autogenID = message.createdTimestamp.toString().split("").reverse().join("");//flips the time stamp
 ...
 * @param {Enumeration} data an enumeration containing all necessary data to do APIfunction calls, and make embeds
 * @param {Discord.Message} message the original message that started this command
 * @param {araay} queryThread the response from FIND_THREAD(data). IMPORTANT: HAS TO BE THE RESPONSE FROM FIND_THREAD
 **/
async function addMessageToThread(data, message, queryThread){
  ...
  return await message.channel.send(addedEmbed).then((msg) => msg.delete(10000));
}

/**
 * @param {Enumeration} data an enumeration containing all necessary data to do APIfunction calls, and make embeds
 * @param {Discord.Message} message the original message that started this command
 * @param {araay} queryThread the response from FIND_THREAD(data). IMPORTANT: HAS TO BE RESPONSE FROM FIND_THREAD
 **/
async function multipleThreadResults(data, message, queryThread){
    /**
     * @var {Enumeration} page This var contains all necessary data to create a "book" of data
     * @var {array} threadListEmbed this array holds each page. Each page is a RichEmbed
     **/
    var page = {
      upperBound: 9,
      lowerBound: 0,
    ...
    queryThread.responseData.slice(page.lowerBound, page.upperBound).forEach(thread => 
      threadListEmbed[page.currentPage].addField(`Thread ID: ${thread.threadID}`, `topic: ${thread.topic}`)
      );
    threadListEmbed[page.currentPage].setFooter(`Page: ${page.currentPage+1}/${page.maxPage+1}`);
    
    //Tell user what they need to do 
    const addMoreDigitsEmbed = new Discord.RichEmbed()
    .setColor('#301934')
    .setTitle(`Add more digits to ID ${data.threadID}`);
    let currentMsg = await message.channel.send(threadListEmbed[page.currentPage]);
    var InstructionMsg = await message.channel.send(addMoreDigitsEmbed);
    ...
    const checkEmoji = async reaction => {
      ...
      //Handles what the current page is and what the range of threads we want is
      ...
      if(change)
        currentMsg.edit(threadListEmbed[page.currentPage]).catch(() => console.log("ERROR MOVING"));
    ...
    filter = (m) => {
      ...
      return (
        newID != null &&    // if regex matches (new id starts with old id) and authors are same then message is collected
    ...
    const collectMessage = async (messageIn) =>
    {
      ...
      let queryThread = await FIND_THREAD(data);
      switch (queryThread.responseData.length) {
        case 0:   await createNewThread(data, message, topic); 
                  break;
        case 1:   await addMessageToThread(data, message, queryThread);
                  break;
        default:  await multipleThreadResults(data, message, queryThread);
