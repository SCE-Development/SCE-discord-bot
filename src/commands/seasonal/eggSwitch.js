...
async function CreateEgg(channel, userID)
{
    let instructionEmbed = new Discord.RichEmbed()
        ...
        .setDescription(`Egg name has to be first
        Each step needs to be entered in separate messages.
        1. Type in your egg name
        OPTIONAL
        2. Type 'Delay: ' then a number for delay between egg spawn. (In milliseconds)
        3. Type 'URL: ' then a discord image URL for egg pic.
        4. Type 'Code: ' then a code so a user can "grab" the egg (Has to be Code or a Delay not both)
    ...
    const eggIDMessage = async messageIn => 
    {
        ...
        {
            let checker = messageIn.content === 'stop' | messageIn.content === 'done';
            ...
            switch(setType)
            {
                case 'delay':
                    ...
                    break;
                case 'url':
                    if(/^https:[\/][\/]cdn.discordapp.com[\/]attachments[\/](.+)/.test(value))
                    {
                        egg.URL = value;
                        infoEmbed.setDescription('image set to:')
                            .setThumbnail(value)
                            .setTitle('URL setting');
                    }
                    else
                    {
                        infoEmbed.setDescription("Make sure it is a url that starts with `https://cdn.discordapp.com/attachments/`"
                        + "To get this URL, you upload a pic to discord then right click on the picture.")
                        .setTitle('Error!');
                    }
                    break;
                case 'code':
                    egg.Code = value;
                    infoEmbed.setTitle('Code setting.')
                        .setDescription(`Code set to \`${egg.Code}\``);
                    break;
                case 'description':
                    egg.Description = value;
                    infoEmbed.setTitle('Description setting.')
                        .setDescription(`description set to \`${egg.Description}\``);
                    break;
                case 'hint':
                    egg.Hint = value;
                    infoEmbed.setTitle('Hint setting.')
                        .setDescription(`Hint set to \`${egg.Hint}\``);
                case 'done':
                    infoEmbed.setTitle('Done')
                        .setDescription(`
                ...
                case 'stop':
                    ...
                    return;
                default:
                    infoEmbed.setTitle('Error')
                        .setDescription('not a valid field');
            }
            channel.send(infoEmbed);
